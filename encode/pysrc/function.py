...
def node_identify(N,information_pos,node_method):
    ...
    if node_method == 'regular_node':
        depth=list(range(n-1))    # [0,1,2,3,4,5,6]
        depth.reverse()
        for i in depth:
            for j in range(2**i):
        ...
        for i in depth:
            for j in range(2**i):
                if node_list[i+1][j*2] == 'rate0' and node_list[i+1][j*2+1] == 'rate0':
                    ...
                    if i == depth[0]:
                        node_list[i][j] = 'spc'
                elif node_list[i+1][j*2] == 'rate0' and node_list[i+1][j*2+1] == 'rate1':
                    if i == depth[0]:
                        node_list[i][j] = 'type'
                elif node_list[i+1][j*2] == 'rate0' and node_list[i+1][j*2+1] == 'type':
                    node_list[i][j] = 'type1'
                elif node_list[i+1][j*2] == 'type' and node_list[i+1][j*2+1] == 'rate1':
                    node_list[i][j] = 'type3'
                elif node_list[i+1][j*2] == 'rate0' and node_list[i+1][j*2+1] == 'type1':
                    node_list[i][j] = 'type1'
                elif node_list[i+1][j*2] == 'type3' and node_list[i+1][j*2+1] == 'rate1':
                    node_list[i][j] = 'type3'
                else:
                    pass
    else:
...
def node_process(up_llr,node_id):
    ...
    if node_id == 'rate0':
        ...
        # llr = bit_reversed(up_llr)
        tmp = np.reshape(up_llr,(2,int(N/2)),order='F')
        sum1 = np.sum(tmp[0])
        sum2 = np.sum(tmp[1])
        if sum1 >= 0:
            b1 = 0
        ...
        if sum2 >= 0:
            ...
            b2 = 1
        bit_depth_i = np.array([b1,b2]*int(N/2))
        bit_depth_n = bit_depth_i*generate_matrix(n) %2
        bit_depth_n = np.array(bit_depth_n)
    elif node_id == 'type3':
        # llr = bit_reversed(up_llr)
        ...
        bit_depth_i =  np.reshape(res,(1,N),order='F')
        bit_depth_n = bit_depth_i*generate_matrix(n) %2
        bit_depth_n = np.array(bit_depth_n)
