...
n = 8
K=2**(n-1)
SNR_para = [2,0.5,5]                                   # snr在bsc为转移概率，在bec为删除概率，都为float64，在awgn为Eb/N0信噪比(dB)，为列表,[start,step,end]
crc_n=0                                                     #有0,4,8,12,16,32，0表示无crc，其他的多项式在CRC里面改

frozen_bit = 0
construction_method = 'pw'                  # 有zw，ga，pw, hpw
channel_con = 'awgn'                        # 有awgn，bsc和bec
decode_method = 'fsc'                        # 有sc，scl, scf, fsc 和 bp            注意bp用的是带扩展因子s的，具体去function.f_hf_SMS看
para1= 4
para2= 'regular_node'
#                                 para1                                                               para2
# SC                              / 忽略                                                              / 忽略
...
# FSC                           / 忽略                                     节点识别，有regular_node, type_node, type_node目前只有type1和type3
#初始值预设完毕
...
np.savetxt("./test_data.csv",np.column_stack((SNR_arr,BER_arr,BLER_arr)),delimiter=",", fmt='%s',header='snr,ber,bler')
